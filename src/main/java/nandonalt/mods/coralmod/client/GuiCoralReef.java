...
public class GuiCoralReef extends GuiScreen {
     ...
     */
    private static final String[] SIZES = new String[] {"small", "normal", "large"};
    ...
    GuiCoralReef(GuiScreen parentGuiScreen) {
    ...
    private String getDesc(final int index) {
        ...
        if(index >= 0 && index < list.size()) {
            ...
            desc = I18n.format("coralmod.settings.unimplemented");
        }
    ...
    public void initGui() {
        ...
        final String s;
        if(isInGame()) {
            s = I18n.format("menu.returnToGame");
        } else {
            s = I18n.format("gui.done");
        }
    ...
    private String getState(int index) {
        ...
        try {
            final int size = Integer.parseInt(state);
            if(size >= 0 && size < SIZES.length) {
                return I18n.format("options.guiScale." + SIZES[size]);
            } else {
            ...
            return bool ? I18n.format("options.on") : I18n.format("options.off");
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float renderPartialTicks) {
        ...
        final String status;
        if(!isInGame()) {
            status = I18n.format("options.title");
