...
public class ReefGen2 implements IReefGen {
	...
	public ReefGen2(Block coralBlock, int numberOfBlocks) {
				...
				for(int k = (int)(d8 - d12 / 2.0); k <= (int)(d8 + d12 / 2.0); k++) {
					for(int m = (int)(d9 - d11 / 2.0); m <= (int)(d9 + d11 / 2.0); m++) {
						...
						&& CoralMod.checkWater(world.getBlock(j, k + 3, m)) && CoralMod.checkWater(world.getBlock(j, k + 4, m))) {
							...
							world.setBlock(j, k + random.nextInt(2), m, coralBlock, 0, 2);
							world.setBlock(j + 1, k + random.nextInt(2), m, coralBlock, 0, 2);
							world.setBlock(j, k + random.nextInt(2), m + 1, coralBlock, 0, 2);
							world.setBlock(j + 1, k + random.nextInt(2), m + 1, coralBlock, 0, 2);
                            generated = true;
						}
					}
				}
